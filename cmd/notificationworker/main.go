...
// Constants for simulating error scenarios for NotificationWorker
const (
	notifyTransientErrorUserID = "user_notify_transient" // User ID to simulate transient notification errors
	notifyCriticalErrorUserID  = "user_notify_fail"      // User ID to simulate critical notification failures
...
func main() {
	...
	log.Info("Starting NotificationWorker...")

	appConfig = AppConfig{
		NatsURL:     natsURL,
		StreamName:  "ORDERS",
		Subject:     "ORDERS.placed",
		QueueName:   "NOTIFICATION_EVENT_QUEUE",
		DurableName: "NOTIFICATION_WORKER_DURABLE",
	...
	consumerConfig := &nats.ConsumerConfig{
		...
		Description:    "Durable consumer for Notification Worker",
	...
	log.Printf("NotificationWorker queue subscribing to NATS subject '%s' with queue group '%s', durable consumer '%s'",
	...
	sub, err := js.QueueSubscribe(
		...
		func(msg *nats.Msg) { // Message handler
			...
			}).Info("Processing order notification")

			// Simulate specific NACK/Term scenarios
			if order.UserID == notifyTransientErrorUserID {
				log.WithFields(logrus.Fields{
					"user_id":       order.UserID,
					"items":         len(order.Items),
					"num_delivered": meta.NumDelivered,
				}).Info("Simulating transient notification error")
			...
			if order.UserID == notifyCriticalErrorUserID {
				log.WithFields(logrus.Fields{
					"user_id": order.UserID,
				}).Info("Simulating critical notification failure")
				// Terminate message for critical errors
				if termErr := msg.Term(); termErr != nil {
					log.WithError(termErr).Error("Error sending Term for critical notification failure")
				}
			...
			log.WithFields(logrus.Fields{
				"user_id": order.UserID,
				"items":   len(order.Items),
			}).Info("Simulating sending notification for order")
	...
	log.Info("NotificationWorker successfully subscribed")
	...
	log.Info("NotificationWorker shut down")
}
